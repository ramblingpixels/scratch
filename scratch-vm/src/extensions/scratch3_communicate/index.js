 ...
 * @constructor
 */
class Scratch3CommunicateBlocks {
    constructor (runtime) {
        /**
    ...
    _clampPenSize (requestedSize) {
        return MathUtil.clamp(
            requestedSize,
            Scratch3CommunicateBlocks.PEN_SIZE_RANGE.min,
            Scratch3CommunicateBlocks.PEN_SIZE_RANGE.max
        );
    }
     ...
     */
    _getPenState (target) {
        let penState = target.getCustomState(Scratch3CommunicateBlocks.STATE_KEY);
        if (!penState) {
            penState = Clone.simple(Scratch3CommunicateBlocks.DEFAULT_PEN_STATE);
            target.setCustomState(Scratch3CommunicateBlocks.STATE_KEY, penState);
        }
        return penState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const penState = sourceTarget.getCustomState(Scratch3CommunicateBlocks.STATE_KEY);
            if (penState) {
                newTarget.setCustomState(Scratch3CommunicateBlocks.STATE_KEY, Clone.simple(penState));
                if (penState.penDown) {
...
module.exports = Scratch3CommunicateBlocks;
